/*
 * RMS-TA Module - Stage 2 Firmware (TA #2) - FIXED v2
 * Board: Arduino Uno
...
#define TA_ADDRESS 2   // this is TA #2
...
void updateCachedTemperature() {
  ...
  if (t != DEVICE_DISCONNECTED_C && t > -55.0f && t < 125.0f) {
    if (!sensorConnected) {
      Serial.println("[TA-2] DS18B20 reconnected - sensor detected again");
    }
    sensorConnected = true;
    cachedTempC = t; // only update on a good read, keep last-known-good otherwise
    Serial.print("[TA-2] Temp: ");
    Serial.print(t, 2);
    Serial.println(" C");
  } else {
    if (sensorConnected) {
      Serial.println("[TA-2] DS18B20 disconnected - sensor not found");
    } else {
      Serial.println("[TA-2] DS18B20 not connected - waiting for sensor");
    }
